...
public:
	...
	void Remove(int InData);						// 특정 데이터를 가지고 있는 노드를 제거하는 함수
	void RemoveAt(int InPoistion);					// 특정 위치의 노드를 제거하는 함수
	ListNode* Search(int InData) const;				// 특정 데이터가 있는 지 확인하는 함수.  리턴이 null이면 없다. null이 아니면 찾은 노드
	void Clear();									// 모든 노드를 제거하는 함수
	void PrintList() const;							// 리스트의 현재 상황을 출력하는 함수
private:
	ListNode* Head = nullptr;	// 노드의 시작(null이면 리스트가 비어있다.)
	ListNode* Tail = nullptr;	// 마지막 노드
	int Size = 0;				// 헤드부터 이어지는 전체 노드의 개수
};
